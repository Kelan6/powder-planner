...
var limitations = require("./limitations");
    ...
    BaseFrequency.prototype.interval = function (rule, current) {
        ...
        if (frequency === 'hourly') {
            ...
            if (hours !== startTimeHours) {
                date += (startTimeHours - hours) * kendo_date_math_1.MS_PER_HOUR;
            }
            date -= start.getTime();
            diff = Math.floor(date / kendo_date_math_1.MS_PER_HOUR);
            ...
            addTime(date, -start);
            diff = Math.round(date / kendo_date_math_1.MS_PER_DAY);
            excess = intervalExcess(diff, interval);
            if (excess !== 0) {
            ...
            var normalizedCurrentIndex = utils_1.normalizeDayIndex(current.getUTCDay(), weekStart);
            var normalizedStartIndex = utils_1.normalizeDayIndex(start.getUTCDay(), weekStart);
            if (normalizedCurrentIndex < normalizedStartIndex) {
                excess += 1;
            }
            excess = intervalExcess(excess, interval);
            if (excess !== 0) {
                utils_1.setDayOfWeek(current, rule.weekStart, -1);
            ...
            excess = intervalExcess(diff, interval);
            if (excess !== 0) {
                day = utils_1.hasModifiers(rule) ? 1 : current.getDate();
...
exports.HourlyFrequency = HourlyFrequency;
/** @hidden */
...
exports.DailyFrequency = DailyFrequency;
/** @hidden */
...
exports.WeeklyFrequency = WeeklyFrequency;
/** @hidden */
...
exports.MonthlyFrequency = MonthlyFrequency;
/** @hidden */
...
exports.YearlyFrequency = YearlyFrequency;
